     ...
     $result3 = mysqli_query( $database, $query3 ) ;
      while($row = mysqli_fetch_row( $result ) and $row2 = mysqli_fetch_row( $result2 ) and $row3 = mysqli_fetch_row( $result3 )){
          for($i = 0; $i < sizeof($row);$i++){
              if($row3[$i] == null){
              echo"$row[$i] <br><form method='POST' action='messagesStaff.php?MessageID={$row2[$i]}'> رسالتك:<input type='text' name='message'><input type='Submit' name='Send' value='ارسل'></form>";
              }
            }
      }
      mysqli_close( $database );      
